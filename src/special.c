...
DECLARE_SPEC_FUN(       spec_dirty              );
DECLARE_SPEC_FUN(	spec_cat		);
/*DECLARE_SPEC_FUN(	spec_bounty		);*/
DECLARE_SPEC_FUN(	spec_sleepy		);
...
DECLARE_SPEC_FUN(       spec_preacher           );
/* the function table */
const   struct  spec_type    spec_table[] =
{
    ...
    {	"spec_cat",			spec_cat		},
/*    {	"spec_bounty",			spec_bounty		},*/
    {	"spec_sleepy",			spec_sleepy		},
...
bool spec_dirty( CHAR_DATA *ch )
{
    ...
    {
	v_next = victim->next_in_room;
	if ( victim != ch && can_see( ch, victim ) && number_bits( 2 ) == 0 
	     && !IS_NPC(victim) && victim->level < 50)
	    break;
    }
    ...
    case 0:
	act( "$n says
G'morn Guvna, You bleedin poof dog lovin patsy!"         
        , ch, NULL, NULL, TO_ROOM );
	return TRUE;

    case 1:
	act( "$n says
Oh wot we got here, a little fairy boy!"
, ch, NULL, NULL, TO_ROOM );
	return TRUE;

    case 2:
	act("$n says
Oh look at the brave adventurer! Im so impressed!",ch,NULL,NULL,TO_ROOM);
	return TRUE;

    case 3:
	act("$n says
Hey Guvna, wot ya go it that little purse", ch,NULL, NULL, TO_ROOM );
	return TRUE;

    case 4:
	act( "$n says
Hey freak, Your mother was an orc!",ch,NULL,NULL,TO_ROOM);
	return TRUE;

    case 5:
	act("$n says
Oh you look so brave, why dont you tie a ribbon around your weapon!"
, ch, NULL, NULL,TO_ROOM );
	return TRUE;

    case 6:
	act("$n says
OOOOH everyone be afraid its a might warrior!",ch,NULL,NULL,TO_ROOM);
    }
...
bool spec_guard( CHAR_DATA *ch )
{
    ...
    if ( victim != NULL )
    {
	sprintf( buf, "%s is a %s! Protect The City!",
	    victim->name, crime );
 	REMOVE_BIT(ch->comm,COMM_NOSHOUT);
	do_yell( ch, buf );
	multi_hit( ch, victim, TYPE_UNDEFINED );
    ...
    if ( ech != NULL )
    {
	act( "$n screams 'We got you now bastard!!",
	    ch, NULL, NULL, TO_ROOM );
...
bool spec_mayor( CHAR_DATA *ch )
{
    ...
    case 'd':
	act( "$n says 'This town is full of scum!'", ch, NULL, NULL, TO_ROOM );
	break;

    case 'e':
	act( "$n says 'I hereby declare the city of Calinth open!'",
	    ch, NULL, NULL, TO_ROOM );
	break;

    case 'E':
	act( "$n says 'I hereby declare the city of Calinth closed!'",
	    ch, NULL, NULL, TO_ROOM );
	break;
...
{ 
  if (chance(10))
  { act( "$n grooms its coat.", ch, NULL, NULL, TO_ROOM );
  }
  else if (chance(10))
  { act( "$n licks its paw daintily.", ch, NULL, NULL, TO_ROOM );
  }
  if(chance(5))
  { act( "$n spits out a hairball.", ch, NULL, NULL, TO_ROOM);
  }
  if (ch->fighting!=NULL && chance(25))
  { act( "$n screeches in anger.",ch, NULL, NULL, TO_ROOM);
  }
return TRUE;
}


bool spec_sleepy( CHAR_DATA *ch)
{
  if (time_info.hour<6 && chance(1))
  {
    act("$n yawns sleepily.",ch,NULL,NULL,TO_ROOM);
  }
  return TRUE;
}
/*
...
*/
bool spec_preacher( CHAR_DATA *ch )
{
    static const char open_path[] =
	"W3a3003b33000c111d0d111Oe333333Oe22c222112212111a1S.";
    ...
    case 'e':
	act( "$n says 'We must lock up the city, to keep out the heathens!",
	    ch, NULL, NULL, TO_ROOM );
	break;
