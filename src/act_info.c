...
to_level = exp_per_level(ch,ch->pcdata->points);
    ...
    sprintf( buf,
"You Are: {W%s{x Who is {W%s {x\n\r{xRank: {W%d{x,\t Age:{W%d{x\t         Sphere:{W%s\r\n",
	ch->name,
	ch->short_descr,
	ch->level, get_age(ch),
    ...
    sprintf(buf, 
"{xRace:{W%s{x \t Sex:{W%s{x \t Guild:{W%s{x\n\r",
   ...
   if(get_curr_stat(ch,STAT_STR) > 18) sstr = 
   "{W Superb {x";
   else if(get_curr_stat(ch,STAT_STR) < 19 && get_curr_stat(ch,STAT_STR)
   > 11 )  sstr = 
   "{W Average{x"; 
   else if(get_curr_stat(ch,STAT_STR) < 12 && get_curr_stat(ch,STAT_STR)
   > 2 ) sstr = 
   "{W Lacking{x";


   if(get_curr_stat(ch,STAT_INT) > 18) sint = 
   "{W Superb {x";
   else if(get_curr_stat(ch,STAT_INT) < 19 && get_curr_stat(ch,STAT_INT)
   > 11 )  sint = 
   "{W Average{x"; 
   else if(get_curr_stat(ch,STAT_INT) < 12 && get_curr_stat(ch,STAT_INT)
   > 2 ) sint = 
   "{W Lacking{x";

   if(get_curr_stat(ch,STAT_WIS) > 18) swis = 
   "{W Superb {x";
   else if(get_curr_stat(ch,STAT_WIS) < 19 && get_curr_stat(ch,STAT_WIS)
   > 11 )  swis = 
   "{W Average{x"; 
   else if(get_curr_stat(ch,STAT_WIS) < 12 && get_curr_stat(ch,STAT_WIS)
   > 2 ) swis = 
   "{W Lacking{x";


   if(get_curr_stat(ch,STAT_DEX) > 18) sdex = 
   "{W Superb {x";
   else if(get_curr_stat(ch,STAT_DEX) < 19 && get_curr_stat(ch,STAT_DEX)
   > 11 )  sdex = 
   "{W Average{x"; 
   else if(get_curr_stat(ch,STAT_DEX) < 12 && get_curr_stat(ch,STAT_DEX)
   > 2 ) sdex = 
   "{W Lacking{x";

   if(get_curr_stat(ch,STAT_CON) > 18) scon = 
   "{W Superb {x";
   else if(get_curr_stat(ch,STAT_CON) < 19 && get_curr_stat(ch,STAT_CON)
   > 11 )  scon = 
   "{W Average{x"; 
   else if(get_curr_stat(ch,STAT_CON) < 12 && get_curr_stat(ch,STAT_CON)
   > 2 ) scon = 
   "{W Lacking{x";
...
*/
    ...
    {
      sprintf (buf, 
	"[{Y%d{x] exp to rank with [%d] total experience.\n\r",
       ((ch->level+1)*to_level)-ch->exp, ch->exp);

      send_to_char( buf, ch );
     }

    sprintf( buf,
"[{W%ld{x Gold]\t  [{W%ld{x Silver]\t    [{W%d{x Trains]\n\r",
	  ch->gold, ch->silver,ch->train );
    send_to_char( buf, ch );

    sprintf( buf,
"[{W%d{x items carried] [{W%ld{x total stones] [{W%d{x max stones]\n\r",
	ch->carry_number,
...
*/
	 ...
	 * Format it up.
	 */
 sprintf( buf, "{x[ {w%6s {x] %s%s%s%s%s\n\r",
...
/*
	    IS_SET(wch->comm, COMM_AFK) ? "[AFK] " : "",
            IS_SET(wch->act, PLR_KILLER) ? "({wWANTED{x) " : "",
            IS_SET(wch->act, PLR_THIEF)  ? "({wWANTED{x) "  : "",
*/
	    wch->short_descr,
...
void do_immwho( CHAR_DATA *ch, char *argument )
{
	 ...
	 * Format it up.
	 */
 sprintf( buf, "{x[ {w%6s %s{x] %s %s %s %s %s %s %s %s\n\r",
	    wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name 
				    : "     ",
	    class,
	    wch->incog_level >= LEVEL_HERO ? "({wIncog{x) " : "",
...
/* Switch skills from 100% to alpha characters */

if ((ch->pcdata->learned[sn]) > 85) skl ="master";
else if ((ch->pcdata->learned[sn]) < 86 && (ch->pcdata->learned[sn])  
> 45) skl ="normal";
else if ((ch->pcdata->learned[sn]) < 46 && (ch->pcdata->learned[sn]) 
> 0) skl ="novice";

sprintf( buf, "{W[{Y%-18s{W]{B %s{x ",
skill_table[sn].name, skl/* ch->pcdata->learned[sn]*/ );
	    send_to_char( buf, ch );
