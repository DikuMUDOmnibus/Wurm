...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
    ...
    if (ch->description[0] != '\0')
    	fprintf( fp, "Desc %s~\n",	ch->description	);
    if (ch->prompt != NULL || !str_cmp(ch->prompt,"<%P%%hp %p%%m %v%%bv>"))
        fprintf( fp, "Prom %s~\n",      ch->prompt  	);
...
/*
	...
	return;
*/
// taking this line out allows keys to be saved
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    ch->comm				= COMM_COMBINE 
					| COMM_PROMPT;
    ch->prompt 				= str_dup( "{c<%P%%hp %p%%m %b%%mv>{x ");
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'B':
	    ...
	    KEY( "Backy",       ch->backy,              fread_string(fp));
            break;
	...
	case 'G':
            KEY( "God",         ch->god, god_lookup(fread_string(fp)));
	...
	case 'P':
	    ...
	    KEY( "Prac",	ch->practice,		fread_number( fp ) );
 	    ...
 	    KEY( "Path",	ch->path,		fread_number( fp ) );
	    break;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// this is new code to implement apartments which can be rented / owned
// began on 3/7/98
// by devbadriel
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

// format for objects saved to rooms
// #O
// Vnum i
// ShD string~
// Desc string~
...
// write contents of a room to a file in area directory
//
void fwrite_apt( )
{
    FILE		*	fp = NULL;
...
fp = fopen("apartments","w");
if(fp==NULL)
  {
    bug("SAVE_LOCKERS: file_error",0);
    return;
  }

    bug("SAVE_LOCKERS: starting save",0);

//only one room exists now add more later
//this is loop for multiple rooms
//room = get_room_index(APT_START);
//while(room->vnum <= 100)

num = 9900;    
room = get_room_index(num);
  {    
    fprintf(fp, "LOCKER %d\n",num);
    obj = room->contents;
    while (obj != NULL)
...
//==================================================
// function to load contents of a locker from a file
// added 4/15/98 by devbadriel
//================================================== 
void fread_apt( )
{
    int 		vnum;
    ...
    bug("LOAD_LOCKERS: loading player lockers",0);

    fp = fopen("apartments","r");
    if(fp == NULL)
      {
	bug("LOAD_LOCKERS: file error",0);
	return;
      }

    word = fread_word(fp);		// starts reading file
    ...
    while(strcmp(word,"END") != 0)
      {
		...
		case 'E':
	    	  if ( !str_cmp( word, "END"))
		    {
		      bug("LOAD_LOCKERS: finished loading",0);
		    }
		...
		case 'L':
        	  if (str_cmp(word,"LOCKER" ) == 0)
          	    {
	    	      rnum = fread_number(fp);
		      room = get_room_index(rnum);
		      if(room == NULL)
	  	    	{
	    	      	  bug("LOAD_LOCKERS: no room number",0);
	    	          fclose(fp);
	                  return;
	  	        }
	  	    }
...
// don't think i need this but leaving it for now
//	          if ( !str_cmp( word, "Vnum" ) )
//	            {
//		      int vnum;
//
