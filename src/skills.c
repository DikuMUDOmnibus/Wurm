...
void check_improve( CHAR_DATA *ch, int sn, bool success, int multiplier )
{
    ...
    newsn = skill_lookup(skill_table[sn].split_name);
    if(ch->pcdata->learned[sn] <= SPLIT &&
       ch->pcdata->learned[newsn] <= 10)
      low = 1;
    if (success)
    {
...
// added code to split skills at SPLIT %
            if(ch->pcdata->learned[sn] > SPLIT && 
		low == 1 &&
		ch->pcdata->learned[newsn] < 10)
	      {
	        sprintf(buf,"You can now learn %s!\n\r",
		    skill_table[sn].split_name);
	        send_to_char(buf,ch);
	        ch->pcdata->learned[newsn] = 10;
	        sprintf(buf,"%s can now learn %s!\n\r",
		    	ch->name, skill_table[sn].split_name);
		log_string(buf);
	      }	    
