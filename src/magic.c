...
void say_spell( CHAR_DATA *ch, int sn )
{
  ...
  sprintf( buf2, "$n says, '%s' in an arcane tongue.", buf );
  sprintf( buf,  "$n says, '%s' in an arcanetongue.",skill_table[sn].name );
...
//    OBJ_DATA *stone;
    ...
    if ( arg1[0] == '\0' )
    {
	send_to_char( "a voice says, 'which what where'?\n\r", ch );
	return;
    }
    ...
    ||   		 ch->pcdata->learned[sn] == 0)))
    {
	send_to_char( "Your spellbook doesnt have that!\n\r", ch );
	return;
    }
  
    if ( ch->position < skill_table[sn].minimum_position )
    {
	send_to_char( "You mind is not clear!\n\r", ch );
	return;
    }
	...
	if ( arg2[0] == '\0' )
	{
	    ...
	    {
		send_to_char( "Cast your magics on what?\n\r", ch );
		return;
	    }
	}
	else
	{
	    ...
	    {
		send_to_char( "Do you see someone i dont?\n\r", ch );
		return;
	    }
	}
	...
	{
	    send_to_char( "Your follower doesnt want that!\n\r",
		ch );
	    return;
	}
...
void spell_galamadriel( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    {
	if (victim == ch)
	  send_to_char("You are already beautiful.\n\r",ch);
	else
	  act("$N is already beautiful.",ch,NULL,victim,TO_CHAR);
	return;
    }
    af.where	 = TO_AFFECTS;
    af.type      = sn;
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
    send_to_char( "You feel the love of the elves.\n\r", victim );
    if ( ch != victim )
	act("$N is protected by your beauty.",ch,NULL,victim,TO_CHAR);
    return;
}
...
void spell_awe( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    {
	if (victim == ch)
          send_to_char("Your presence is already felt.\n\r",ch);
	else
          act("$N already has great presence.",ch,NULL,victim,TO_CHAR);
	return;
    }
    af.where	 = TO_AFFECTS;
    af.type      = sn;
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
    send_to_char( "You feel your awe protecting you.\n\r", victim );
    if ( ch != victim )
        act("$N eminates a great awe.",ch,NULL,victim,TO_CHAR);
    return;
}
...
void spell_combat_mind( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    {
	if (victim == ch)
          send_to_char("You already have a combatmind.\n\r",ch);
	else
          act("$N already has a combatmind.",ch,NULL,victim,TO_CHAR);
	return;
    }
    ...
    send_to_char( "You feel like a veteran warrior.\n\r", victim );
    if ( ch != victim )
        act("You grant $N a combatmind.",ch,NULL,victim,TO_CHAR);
    return;
}

void spell_mind_bar( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    CHAR_DATA *victim;
    AFFECT_DATA af;
    victim = (CHAR_DATA *) vo;
    ...
    {
	if (victim == ch)
          send_to_char("You already have a mind bar.\n\r",ch);
	else
          act("$N already has a mind bar.",ch,NULL,victim,TO_CHAR);
	return;
    }
    ...
    send_to_char( "You feel mentally powerful.\n\r", victim );
    if ( ch != victim )
        act("You grant $N a mind bar.",ch,NULL,victim,TO_CHAR);
    return;
}
...
void spell_blindness( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    affect_to_char( victim, &af );
    send_to_char("You feel nauseus as the spell hits you\n\r",victim);
...
void spell_project_force(int sn,int level,CHAR_DATA *ch, void *vo,int target)
{
    ...
    act("A force projection leaps from $n's hand and arcs to $N.",
        ch,NULL,victim,TO_ROOM);
    act("A force projection leaps from your hand and arcs to $N.",
	ch,NULL,victim,TO_CHAR);
    act("A force projection leaps from $n's hand and hits you!",
	ch,NULL,victim,TO_VICT);  
	     ...
	     tmp_vict = next_vict) 
	{
	  ...
	  if (!is_safe_spell(ch,tmp_vict,TRUE) && tmp_vict != last_vict)
	  {
	    found = TRUE;
	    last_vict = tmp_vict;
            act("The projected force arcs to $n!",tmp_vict,NULL,NULL,TO_ROOM);
            act("The projected force hits you!",tmp_vict,NULL,NULL,TO_CHAR);
	    dam = dice(level,6);
	    if (saves_spell(level,tmp_vict,DAM_LIGHTNING))
		dam /= 3;
	    damage_old(ch,tmp_vict,dam,sn,DAM_LIGHTNING,TRUE);
	...
	if (!found) /* no target found, hit the caster */
	{
	  ...
	  if (last_vict == ch) /* no double hits */
	  {
            act("The projected force seems to die out.",ch,NULL,NULL,TO_ROOM);
            act("The projected force seems to die out.",
		ch,NULL,NULL,TO_CHAR);
	    return;
	  }
	
	  last_vict = ch;
          act("The projected force arcs to $n...whoops!",ch,NULL,NULL,TO_ROOM);
          send_to_char("You are struck by your own force projection!\n\r",ch);
	  dam = dice(level,6);
	  if (saves_spell(level,ch,DAM_LIGHTNING))
	    dam /= 3;
	  damage_old(ch,ch,dam,sn,DAM_LIGHTNING,TRUE);
...
void spell_domination( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if ( victim == ch )
    {
        send_to_char( "You look deep into your own eyes!\n\r", ch );
	return;
    }
    ...
    {
	send_to_char(
            "The Overlord does not allow charming in the city limits.\n\r",ch);
	return;
    }
    ...
    af.bitvector = AFF_CHARM;
    affect_to_char( victim, &af );
    act( "Doesnt $n have mesmerizing eyes?", ch, NULL, victim, TO_VICT );
    if ( ch != victim )
        act("$N is dominated by your power.",ch,NULL,victim,TO_CHAR);
    return;
}
...
void spell_aurasight( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if ( IS_AFFECTED(victim, AFF_DETECT_GOOD) )
    {
        if (victim == ch)
          ...
          act("$N can already see aura's.",ch,NULL,victim,TO_CHAR);
        return;
    }
    af.where     = TO_AFFECTS;
    af.type      = sn;
...
void spell_life_detect(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    {
        if (victim == ch)
          send_to_char("Your senses are at their max. \n\r",ch);
        else
          act("$N senses are at their max.",ch,NULL,victim,TO_CHAR);
        return;
    }
    af.where     = TO_AFFECTS;
    af.type      = sn;
...
void spell_detect_invis( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if ( IS_AFFECTED(victim, AFF_DETECT_INVIS) )
    {
       ...
       send_to_char("You begin chanting.\n\r",ch);
       act("$n begins chanting.",ch,NULL,victim,TO_ROOM);
...
void spell_poison_sense( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    OBJ_DATA *obj = (OBJ_DATA *) vo;
...
void spell_life_drain( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if ( saves_spell( level, victim,DAM_NEGATIVE) )
    {
        send_to_char("You feel a slight tug at your soul.\n\r",victim);   
	return;
    }
    ...
    send_to_char("You feed on the soul of your victim\n\r",ch);
    damage_old( ch, victim, dam, sn, DAM_NEGATIVE ,TRUE);
...
void spell_levitate( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    {
	if (victim == ch)
          send_to_char("Your levitation is at its max.\n\r",ch);
	else
          act("$N is flying or levitating already.",ch,NULL,victim,TO_CHAR);
	return;
    }
    af.where     = TO_AFFECTS;
    af.type      = sn;
    ...
    af.bitvector = AFF_FLYING;
    affect_to_char( victim, &af );
    send_to_char( "Your slowly float upwards.\n\r", victim );
    act( "$n's slowly floats upwards.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_adrenalin(int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    {
	if (victim == ch)
          send_to_char("You are already pumped with adrenalin.\n\r",ch);
	else
          act("$N is already pumped with adrenalin.",ch,NULL,victim,TO_CHAR);
	return;
    }
    ...
    send_to_char("You are filled with adrenalin!\n\r",victim);
    act("$n gets a wild look in $s eyes!",victim,NULL,NULL,TO_ROOM);
}
...
void spell_cell_adjust(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    {
	if (victim == ch)
          send_to_char("Your cells are already powerful!\n\r",ch);
	else
          act("$N's cells cant get any stronger.",ch,NULL,victim,TO_CHAR);
	return;
    }
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
    send_to_char( "Your cells adjust for added strength!\n\r", victim );
    act("$n's cells adjust for added strength.",victim,NULL,NULL,TO_ROOM);
    return;
}
...
void spell_complete_heal( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    victim->hit = UMIN( victim->hit + 100, victim->max_hit );
    update_pos( victim );
    send_to_char( "A sense of warmth fills your body.\n\r", victim );
    if ( ch != victim )
	send_to_char( "Ok.\n\r", ch );
    return;
}
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	switch (obj->value[0])
	{
	    ...
	    case(WEAPON_STAFF)	: send_to_char("spear/staff.\n\r",ch);  break;
...
void spell_object_lore( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    OBJ_DATA *obj = (OBJ_DATA *) vo;
    char buf[MAX_STRING_LENGTH];
    AFFECT_DATA *paf;
...
void spell_true_path( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    char buf[MAX_INPUT_LENGTH];
    BUFFER *buffer;
    OBJ_DATA *obj;
    ...
    if ( !found )
        send_to_char( "You fail to find the true path.\n\r", ch );
    else
	page_to_char(buf_string(buffer),ch);
...
void spell_true_pain( int sn, int level, CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    static const sh_int dam_each[] = 
    {
...
void spell_drunken( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim;
    AFFECT_DATA af;
    ...
    if ( saves_spell( level, victim,DAM_POISON) )
    {
        act("$n looks a bit drunk, but it passes.",victim,NULL,NULL,TO_ROOM);
        send_to_char("You feel a bit drunk, but it passes.\n\r",victim);
	return;
    }
    ...
    af.bitvector = AFF_POISON;
    affect_join( victim, &af );
    send_to_char( "You feel poisoned by alcohol.\n\r", victim );
    act("$n looks extremely drunk.",victim,NULL,NULL,TO_ROOM);
    return;
}
...
void spell_control_body( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    {
	if (victim == ch)
          send_to_char("You body is dense already.\n\r",ch);
	else
          act("$N is already dense as possible.",ch,NULL,victim,TO_CHAR);
	return;
    }
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
    act( "$n's body becomes very dense.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "Your body becomes very dense.\n\r", victim );
    return;
}
...
void spell_flesh_armor( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	if (victim == ch)
          ...
          act("$N's flesh is already like armor.",ch,NULL,victim,TO_CHAR);
	return;
    }
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
    act( "$n's skin turns into armor.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "Your skin turns to armor.\n\r", victim );
    return;
}
...
void spell_astral_walk( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    && ( saves_spell( level - 5, victim,DAM_OTHER))))
    {
        send_to_char( "You failed to astral walk.\n\r", ch );
	return;
    }
    ...
    if (victim != ch)
        send_to_char("You have been flung through the astral plane\n\r",victim);
...
void spell_animate( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if( obj->item_type != ITEM_CORPSE_NPC )
    {
     ...
     if( obj->item_type == ITEM_CORPSE_PC )
         send_to_char("Not on dead players!\n\r",ch);
         else
    ...
    if ( obj->level > (ch->level +10 ) )
    {
      ...
      send_to_char("You cant control that great spirit.\n\r",ch);
      return;
    }
    if (ch->pet != NULL)
    {
...
void spell_guards( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *mob;
    int i;
...
if (ch->pet != NULL)
{
send_to_char("\n\rYou already control a guard!\n\r",ch);
return;
}

mob = create_mobile( get_mob_index( MOB_VNUM_GUARD ));
mob->level                    =ch->level*.8;
...
mob->leader = ch;
ch->pet = mob;
do_say( mob, "How may i serve you my lord!" );
return;
}

void spell_shadowpet( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *mob;
    int i;
...
if (ch->pet != NULL)
{
send_to_char("\n\rYou already control a shadowpet!\n\r",ch);
return;
}

mob = create_mobile( get_mob_index( MOB_VNUM_SHADOWPET ));
...
mob->leader = ch;
ch->pet = mob;
do_say( mob, "I am your shadowpet my lord!" );
return;
}
