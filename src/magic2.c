...
void spell_ghost( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
...
{
if (victim == ch)
send_to_char("already testing\n\r",ch);
else
act("$N can alrady snse a test flag",ch,NULL,victim,TO_CHAR);
return;
}
...
af.bitvector  =AFF_GHOST;
affect_to_char( victim, &af );
send_to_char("test affect on\n\r",ch);
if ( ch != victim )
send_to_char("ok\n\r",ch);
return;
}
