...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
    ...
    if ( ch->pcdata->condition[iCond] <= 20 &&
         ch->pcdata->condition[iCond] > 10 )
    {
	switch ( iCond )
	{
	case COND_HUNGER:
	...
	case COND_DRUNK:
	    if ( condition != 0 )
	      {
		send_to_char( "You are wasted.\n\r", ch );
	        ch->move = ch->move - (ch->move * 25/100);
	      }
	    break;
	}
    } 
   ...
   if ( ch->pcdata->condition[iCond] <= 10 &&
	...
	case COND_HUNGER:
	    ...
	    ch->move = ch->move - (ch->move * 15/100);
	    break;
   ...
   if ( ch->pcdata->condition[iCond] <= 5 )
	...
	{
	case COND_HUNGER:
 	    act("$n is starving and will die soon if not aided.", 
		ch, NULL, NULL, TO_ROOM);
	    send_to_char( "You are starving.\n\r",  ch );
	    ch->hit = ch->hit - (ch->hit * 30/100);
	    ch->move = ch->move - (ch->move * 30/100);
	    break;

	case COND_THIRST:
	    send_to_char( "You are descicated.\n\r", ch );
	    ch->hit = ch->hit - (ch->hit * 30/100);
	    ch->move = ch->move - (ch->move * 30/100);
	    break;
...
/* THIS FINISHES OFF A CHARACTER WHO IS STARVING / DESCICATED */
      if(ch->hit < 10 && ch->fighting == NULL)  
        { 
 	  ch->hit = ch->hit - 2;
	  if(ch->hit < -1)
 	    {
 	      act("$n has starved to death.", ch, NULL, NULL, TO_ROOM);
	      send_to_char(
	   " You have died of starvation, buy food next time.\n\r",ch);
...
void weather_update( void )
{
	...
	{
	    strcat( buf, "Dark clouds gather in the sky.\n\r" );
	...
	{
	    strcat( buf, "Sheets of rain soak your clothes.\n\r" );
	    weather_info.sky = SKY_RAINING;
	}

	if ( weather_info.mmhg > 1030 && number_bits( 2 ) == 0 )
	{
	    strcat( buf, "Dissipating clouds catch your eye.\n\r" );
	    weather_info.sky = SKY_CLOUDLESS;
	}
	break;
	...
	{
	    strcat( buf, "Blue streaks of lightning rage above.\n\r" );
	...
	|| ( weather_info.mmhg > 1010 && number_bits( 2 ) == 0 ) )
	{
	    strcat( buf, "The bone chilling rain has stopped.\n\r" );
	    weather_info.sky = SKY_CLOUDY;
	}
	break;
	...
	|| ( weather_info.mmhg >  990 && number_bits( 2 ) == 0 ) )
	{
	    strcat( buf, "The fierce lightning disappears.\n\r" );
	    weather_info.sky = SKY_RAINING;
	    break;
	}
	break;
...
{   
	...
	case ITEM_EXPLOSIVE:  message = "$p explodes violently.";
			do_explode(obj);
				break;
...
void underwater_update( void )
{
	...
	ch->hit /= 2;
	send_to_char("{BYour Drowning!!{x\n\r",ch);
	}
	else
	{
	ch->hit = 1;
	raw_kill(ch);
	send_to_char("{BYou have drowned!!{x\n\r",ch);
	}
       }
      }
}
...
if(obj->carried_by!=NULL){
send_to_char("You see a bright flash of light!\n\r",obj->carried_by);
send_to_char("BBBBBOOOOOOOOOOOOMMMMMM!!!!!!!!\n\r",obj->carried_by);
obj->carried_by->hit-=(obj->carried_by->hit)/(number_range(2,5));		
act( "$n explodes in a bright flash of light!!",
		obj->carried_by,obj,NULL,TO_ROOM );	
		}
	
	if(obj->carried_by==NULL){
   act( "The $p explodes in a bright flash of light!!",NULL,obj,NULL,TO_ROOM );	
	ch=obj->carried_by;		
  to=obj->in_room->people;
/*    while(to!=NULL){
	to->hit-=(to->hit)/(number_range(3,7));
send_to_char("BBBBOOOOOOOOMMMMMMM!!!!!!\n\r",to);		
send_to_char("You are thrown to the floor\n\r",to);		
to=to->next_in_room;
	}
  */       
