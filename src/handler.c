...
/*
 * function to initialize last 10 player list
 */
...
/*
 * function to insert new player in last 10 list
 */
...
//  int i;
    if(ch->level < 59)
      {
        log[head].log_name = ch->name;
        strcpy(log[head].buffer,ch->name);
        log[head].race = ch->race;
        log[head].level = ch->level;
        head++;
        if(head == 10)
          head = 0;
        return head;
...
/*
 * function to display last 10 people to log in
 */
void show_player_log(CHAR_DATA *ch, LOG_TYPE *log, int head)
  {
	...
	"The last 10 people to enter this realm were\n\n",46);  
    write_to_buffer(ch->desc,"NAME\t\t\tRACE\t\tLEVEL\n",25);
    write_to_buffer(ch->desc,
        "-------------------------------------------------\n",51);
    for(i=0;i<10;i++)
      {
	if(log[i].level > 0)
	  {
            sprintf(buf,"%s\t\t\t%s\t\t%d",
		    log[i].buffer, 
		    race_table[log[i].race].name,
		    log[i].level);
	    write_to_buffer(ch->desc,buf,50);
	    write_to_buffer(ch->desc,"\n",2);
	  }
      }  
    write_to_buffer(ch->desc,"\n\n",4);
...
int hometown_lookup (const char *town)
{
  if(!str_prefix(town,"calinth"))
  return 1;
 
  if (!str_prefix(town,"new thalos"))
  return 2;
  
  if (!str_prefix(town,"ofcol"))
  return 3;

  if (!str_prefix(town,"malathar"))
  return 4;

  if (!str_prefix(town,"calinth"))
  return 5;

  else if (!str_prefix(town,"calinth"))
  return 6;
  else
  return -1;
}
...
int apply_ac( OBJ_DATA *obj, int iWear, int type )
{
    ...
    case WEAR_BELT:     return     obj->value[type];
   }
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    */
    ...
    {
	char_to_room(ch,get_room_index(ROOM_VNUM_DEMISE));
	return;
    }
return;
}
...
char *affect2_bit_name( int vector )
{
    ...
    buf[0] = '\0';
    if ( vector & AFF_GHOST         ) strcat( buf, " ghost"         );
...
char *wear_bit_name(int wear_flags)
{
    ...
    if (wear_flags & ITEM_WEAR_BELT     ) strcat(buf, " sheath");
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
