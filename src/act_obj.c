...
void do_sheath( CHAR_DATA *ch, char *argument )
{
    ...
    if ( CAN_WEAR( obj, ITEM_WEAR_BELT ) )
    {
      ...
      {
	send_to_char("You need a belt to sheath your weapon\n\r",ch);
        return;
      }
     
	act( "$n sheaths $p in $s sheath.",  ch, obj, NULL, TO_ROOM);
	act( "You sheath $p in your sheath.", ch, obj, NULL, TO_CHAR);
	equip_char( ch, obj, WEAR_BELT );
	return;
      }
...
void do_draw( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( obj = get_obj_wear( ch, arg ) ) == NULL )
    {
        send_to_char( "That item is not sheathed.\n\r", ch );
	return;
    }

    remove_obj( ch, obj->wear_loc, TRUE );
    act( "$n draws $s $p",ch, obj, NULL, TO_ROOM );
...
void do_push_drag( CHAR_DATA *ch, char *argument, char *verb )
{
  ...
  else
  {
    if ( ch == victim )
    {
       send_to_char("You cant drag or push yourself!\n\r",ch);
	return;
    }
