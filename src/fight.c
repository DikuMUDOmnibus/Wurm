...
/*****************************************/
/*	GODS FIGHTING MESSAGE		 */
    
        if (IS_IMMORTAL(ch) &&
            IS_IMMORTAL(victim) &&
	    dam > 50)
	   {
	     act( "The world rocked by the anger of the gods.",
...
dam_type, bool show ) {
    ...
    && victim->in_room!=ch->in_room)
	  ch->hunting=get_char_area(ch,victim->name);
...
*/
	...
	/* player doing the killing */
	...
	
     {
           ...
           if (ch->level < 10 )
	    {
		send_to_char("You cant PK til level 10.\n\r",ch);
		return TRUE;
	    }

	   if (ch->level > victim->level + 5 )
	    {
		send_to_char("PK Within five levels only.\n\r",ch);
		return TRUE;
	    }
	}
    }
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( ch->master == NULL )
	{
        ...
        send_to_char( "An onlooker gasps an runs for
help!\n\r",ch->master);
    ...
    ||	 ch->fighting  == victim)
	return;
    send_to_char( "An onlooker gasps and runs for help!\n\r",ch);
    send_to_char( "You are now WANTED!\n\r", ch );
...
void death_cry( CHAR_DATA *ch )
{
	...
	obj->timer	= number_range( 4, 7 );
	 if(vnum == OBJ_VNUM_SEVERED_HEAD)
		{
		if(!IS_NPC(ch)){
		obj->timer=number_range(70,90);
		} else {
		obj->timer=number_range(40,70);
		}	
		}
...
*/
	...
	sprintf( buf, "You gain some experience.\n\r");
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim, int total_levels )
{
	...
	case -8 :	base_exp =   7;		break;
	case -7 :	base_exp =  10;		break;
	case -6 : 	base_exp =  15;		break;
	...
	case  3 :	base_exp = 105;		break;
	case  4 :	base_exp = 110;		break;
    } 
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune )
{
 ...
 else if ( dam <=   1 ) { vs = "{knick{x"; vp="{knicks{x"; }
 else if ( dam <=   3 ) { vs = "{rscratch{x"; vp="{rscratches{x"; }
 else if ( dam <=   4 ) { vs = "{rgraze{x"; vp ="{rgrazes{x"; }
 else if ( dam <=   8 ) { vs = "{rslash{x";	vp = "{rslashes{x";}
 else if ( dam <=  12 ) { vs = "{rcut into{x";	vp = "{rcuts into{x";}
    ...
    else if ( dam <=  60 ) { vs = "{r DEMOLISH{x ";
			     vp = "{r DEMOLISHES{x ";}
    else if ( dam <=  75 ) { vs = "{r DEVASTATE{x ";
			     vp = "{r DEVASTATES{x ";}
    else if ( dam <= 100)  { vs = "{r OBLITERATE{x ";
			     vp = "{r OBLITERATES{x ";		}
    else if ( dam <= 125)  { vs = "{r ANNIHILATE{x ";
			     vp = "{r ANNIHILATES{x ";		}
    else if ( dam <= 150)  { vs = "{r ERADICATE{x ";
			     vp = "{r ERADICATES{x ";}
    else if ( dam <= 200)  { vs = "{r CRUCIFY{x ";
			     vp = "{r CRUCIFIES{x ";}
    else                   { vs = "do{r UNSPEAKABLE{x things to";
			     vp = "does{r UNSPEAKABLE{x things to";}
...
if ( ( obj = get_eq_char( ch, WEAR_WIELD ) ) == NULL)
{
  send_to_char("You need a weapon to circle!\n\r",ch);
  return;
}

if ( ( victim = ch->fighting ) == NULL)
{
  send_to_char("Your not even fighting!\n\r",ch);
  return;
}
...
void do_round( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;

    if ( !IS_NPC(ch)
    &&   ch->level < skill_table[gsn_round].skill_level[ch->class] )
    {
	send_to_char(
	    "You better leave the martial arts to fighters.\n\r", ch );
	return;
    ...
    WAIT_STATE( ch, skill_table[gsn_round].beats );
    if ( get_skill(ch,gsn_round) > number_percent())
    {
send_to_char("You deliver an incredible roundhouse!\n\r",ch);
damage(ch,victim,number_range( 2, ch->level ), gsn_round,DAM_BASH,TRUE);
	check_improve(ch,gsn_round,TRUE,1);
    }
    else
    {
	damage( ch, victim, 0, gsn_round,DAM_BASH,TRUE);
	check_improve(ch,gsn_round,FALSE,1);
    }
    return;
}
